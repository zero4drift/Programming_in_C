...
{
	int status;
	char testStr[81] = "the wrong son";
	char testSubStr[] = "wrong";
	char testReplaceStr[] = "very right";
	int replaceString(char str[], char subStr[], char replaceStr[]);
	
	printf("Before replaced: %s\n", testStr);
	
	status = replaceString(testStr, testSubStr, testReplaceStr);
	
	if(status == 0)
		printf("After \"%s\" replaced with \"%s\": %s\n", testSubStr, testReplaceStr, testStr);
	else
		printf("Failed, \"%s\" need to be replaced is not in \"%s\".\n", testSubStr, testStr);
